...
class sql_commands extends Connect
{
	...
	function add_pro($componet,$price,$descrip,$categoryy)
	{
		$rows;
	...
	function select_all_products()
	{
		...
		if($num_row>0)
		{
			for($a=0; $a<$num_row; $a++)
			{
				...
				$msg.=$id." ".$pro." "."<a href='edit.php?edit=$comp_id'>edit</a> <a href='Inventory.php?delete=$comp_id'>Delete</a>"."<br/>";
			}
		}
		return $msg;
	}
	...
	function order_item($order_id,$session)
	{
		...
		foreach($session as $name =>$value)
		{
			if($value>0)
			{
				...
				{
					$comp_id=substr($name,5,(strlen($name)-5));
					$query="select * from comp where comp_id=$comp_id";
					$result=$this->db->query($query);
					$rows=$result->fetch_assoc();
					$comp_name=$rows['comp_name'];
